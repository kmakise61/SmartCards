...
const Flashcard: React.FC<FlashcardProps> = ({ card, isFlipped, onFlip }) => {
  return (
    ...
    >
      ...
      >
        {/* FRONT */}
        <div className="absolute inset-0 w-full h-full backface-hidden flex flex-col glass-panel sharp-card shadow-2xl">
          ...
          <div className="flex-1 p-8 md:p-12 flex flex-col justify-center items-center overflow-y-auto custom-scrollbar">
            <div className="mb-8 md:mb-12">
              <span className="px-3 py-1 border border-accent/20 text-accent text-[9px] font-black uppercase tracking-[0.2em] rounded-full bg-accent/5">
                Inquiry
              </span>
            </div>
            <div className="font-medium leading-relaxed tracking-tight w-full flex-1 flex flex-col justify-center">
              <FormattedText text={card.front} align="center" size="xl" />
            </div>
            {card.hint && (
          ...
          <div className="p-4 border-t border-border-color text-center text-[8px] font-black uppercase tracking-[0.3em] opacity-30">
            Tap to Reveal
          </div>
        </div>

        {/* BACK */}
        <div className="absolute inset-0 w-full h-full backface-hidden rotate-y-180 flex flex-col glass-panel sharp-card shadow-2xl bg-gradient-to-br from-surface via-panel to-surface">
          <div className="absolute top-0 inset-x-0 h-0.5 bg-gradient-to-r from-accent to-purple-600" />
          ...
          <div className="flex-1 p-8 md:p-12 flex flex-col items-center overflow-y-auto custom-scrollbar">
            ...
            <div className="w-full flex-1 flex flex-col justify-center">
              <FormattedText text={card.back} align="left" size="lg" />
            </div>
            ...
            {card.tags.length > 0 && (
              <div className="mt-8 pt-6 border-t border-border-color w-full flex flex-wrap gap-2 justify-center">
                {card.tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-2.5 py-1 rounded-md border border-border-color bg-panel text-[8px] font-black text-text-secondary uppercase tracking-widest"
                  >
                    #{tag}
                  </span>
                ))}
